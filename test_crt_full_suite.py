...
def run():
    ...
    from bb_crt import jc_mul, solve_matching_at_component, jc_is_unit, jc_basis, jc_to_f2_vector
    import numpy as np
    D, K1, K2 = 1, 4, 4
    ...
    # additionally check the paper's own stated quantitative detail: "the 32
    # stacked linear equations in 16 unknowns are consistent (a 2-dimensional
    # solution space, matching dim Jc/<Ac,Bc> = 2)"
    basis = jc_basis(D, K1, K2, galois.GF(2))
    assert len(basis) == 16, "expected 16 unknowns (dim J_c)"
    cols = [np.concatenate([jc_to_f2_vector(jc_mul(e, Ac2, K1, K2), D, K1, K2),
                             jc_to_f2_vector(jc_mul(e, Bc2, K1, K2), D, K1, K2)])
            for e in basis]
    Mmat = np.array(cols, dtype=np.uint8).T   # rows=2*D*K1*K2 equations, cols=D*K1*K2 unknowns
    assert Mmat.shape == (32, 16), f"expected 32 equations x 16 unknowns, got {Mmat.shape}"
    from bb_code import gf2_rref_rank
    quotient_dim = 16 - gf2_rref_rank(Mmat)
    assert quotient_dim == 2, f"expected dim J_c/<Ac,Bc>=2, got {quotient_dim}"
    print("  OK: (u,v) -> no unit [matches paper]; (u+v,uv) -> unit found and independently "
          "re-verified [matches paper exactly, including the 32-eq/16-unknown/dim-2 details]")
