...
and Local Transfer Data" (outline, Sec. 2.2).

A polynomial in R_{l,m} is represented as a dense l x m numpy array over F2
(entry [i,j] = coefficient of x^i y^j).

This module also implements the ring-automorphism catalog of Sec. 2.2:
...
# ============================================================================
#  Ring automorphism catalog  (Sec. 2.2 of the outline)
# ============================================================================

@dataclass(frozen=True)
class RingAuto:
    """A ring automorphism of R_{l,m}, given as an exponent-lattice map."""
    name: str
...
# ============================================================================
#  Genuine group closure (BFS), shared by forward cataloging (full_catalog)
#  AND inverse design (bb_automorphisms / inverse_design orbit machinery).
#  This is the "shared algebraic structure" referenced throughout: both the
#  forward-problem catalog and the inverse-problem orbit-closure construction
#  are literally built on the same permutation-group closure routine below,
#  rather than each maintaining its own (previously divergent, and in the
#  forward case, incomplete) notion of "the automorphism group".
# ============================================================================

def _perm_of(phi: "RingAuto", ell: int, m: int) -> dict:
    return {(i, j): (phi.map(i, j)[0] % ell, phi.map(i, j)[1] % m)
            for i in range(ell) for j in range(m)}


def _compose_perm(p: dict, q: dict) -> dict:
    """p after q."""
    return {k: p[v] for k, v in q.items()}


def generate_group(generators: List["RingAuto"], ell: int, m: int) -> List["RingAuto"]:
    """
    BFS closure of the subgroup of Sym(Z_l x Z_m) generated by `generators`,
    returned as genuine RingAuto objects (with provenance-tracked names, so
    downstream code can still classify hits by which primitive generators
    were involved). Unlike a fixed "one multiplier, one fold, one shear"
    template, this is closed under composition by construction: every
    element reachable by composing any sequence of generators is included.
    """
    ident_perm = {(i, j): (i, j) for i in range(ell) for j in range(m)}
    ident = RingAuto("id", lambda i, j: (i, j))
    seen = {tuple(sorted(ident_perm.items())): ident}
    frontier = [(ident_perm, ident)]
    gen_pairs = [(_perm_of(g, ell, m), g) for g in generators]
    while frontier:
        nxt = []
        for gperm, gauto in frontier:
            for hperm, hauto in gen_pairs:
                newperm = _compose_perm(hperm, gperm)  # apply gauto then hauto
                key = tuple(sorted(newperm.items()))
                if key not in seen:
                    newauto = RingAuto(f"({hauto.name})o({gauto.name})",
                                        lambda i, j, p=newperm, e=ell, mm=m: p[(i % e, j % mm)])
                    seen[key] = newauto
                    nxt.append((newperm, newauto))
        frontier = nxt
...
def full_catalog(ell: int, m: int, include_shears: bool = True) -> List[RingAuto]:
    """
    The genuine subgroup of Aut(R_{l,m}) generated by the full Sec. 2.2
    catalog: all multipliers, {theta_x, theta_y, iota}, both shear families
    (x -> x y^c and its transpose y -> y x^c'), and tau when l == m --
    obtained by BFS closure (generate_group), not by a fixed composition
    template. This guarantees full coverage: every element reachable by
    composing catalog generators in any order/multiplicity is included.
    """
    gens: List[RingAuto] = [multiplier(ell, m, jx, jy) for (jx, jy) in unit_multipliers(ell, m)]
    gens += [theta_x(), theta_y(), full_fold()]
    if include_shears:
        gens += shear_family(ell, m) + shear_family_transpose(ell, m)
    if ell == m:
        gens.append(transpose_auto())
    return generate_group(gens, ell, m)
