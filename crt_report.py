...
def report_forward(name, ell, m, A_terms, B_terms):
    ...
    for key in ["ACCEPTED", "step3_support_mismatch", "step4_jacobian_rank_mismatch",
                "step4.5_linear_order_mismatch", "step5_no_local_unit"]:
        if key in counts:
    ...
    pruned = (counts.get("step3_support_mismatch", 0) + counts.get("step4_jacobian_rank_mismatch", 0)
              + counts.get("step4.5_linear_order_mismatch", 0))
    print(f"  -> {pruned}/{total} candidates rejected by cheap prefilters (steps 3-4.5), "
          f"before the expensive per-component solve (step 5)")
