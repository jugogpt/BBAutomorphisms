...
def jc_inverse(a, K1: int, K2: int):
    """
    Invert a unit a in J_c = F_{2^D}[u,v]/<u^K1,v^K2> (a[0,0] != 0), via
    Newton iteration for inverses in a local ring: b_{k+1} = b_k*(2-a*b_k)
    (char 2: 2-a*b_k = a*b_k, so b_{k+1}=b_k*(1+a*b_k) -- but written via
    the standard doubling-precision Newton step for clarity/portability).
    Doubles the number of correct terms of the u,v-adic expansion each
    step, so converges in O(log2(K1*K2)) iterations (finite, since the
    maximal ideal <u,v> is nilpotent -- (K1-1)+(K2-1)+1 kills any element
    of it). Returns None if a is not a unit (a[0,0]==0).
    """
    field = type(a)
    if a[0, 0] == 0:
        return None
    b = field.Zeros((K1, K2))
    b[0, 0] = a[0, 0] ** -1          # correct mod the maximal ideal (order-0 term)
    precision = 1
    target = K1 * K2                 # more than enough u,v-adic terms to be exact
    while precision < target:
        ab = jc_mul(a, b, K1, K2)
        # char 2: b_new = b*(2 - a*b) = b*(-a*b) = b*a*b  (since 2=0, -1=1)
        # equivalently, and more transparently: correction = 1 + a*b (in
        # char 2, "2 - a*b" and "a*b" differ only by the sign convention;
        # use the standard identity b_new = b + b*(1 - a*b) = b*(1+ab)+b... )
        one = field.Zeros((K1, K2))
        one[0, 0] = field(1)
        err = one + ab               # 1 - a*b, char 2
        b = b + jc_mul(b, err, K1, K2)
        precision *= 2
    return b


def linear_order_prefilter(A_c, B_c, X1, X2, K1: int, K2: int):
    """
    A cheap NECESSARY condition strictly stronger than Prop. 3.10's rank
    check (Sec. 3.2.1), derived directly from the Hasse-Taylor jet: at a
    SUPPORTED component (both A_c,B_c have zero constant term), the
    matching equation omega.A_c=X1, omega.B_c=X2, restricted to its
    LINEAR-order (u,v-coefficient) part, depends ONLY on omega's own
    constant term omega_0 -- because omega's higher-order terms can only
    multiply against A_c,B_c's linear-or-higher terms, landing at
    quadratic order or above (verified directly, not just asserted, in
    the exploration that produced this function). So:
        omega_0 . (A_u,A_v) = (X1_u,X1_v),   omega_0 . (B_u,B_v) = (X2_u,X2_v)
    is a single-scalar (dimension-D, not D*K1*K2) linear consistency check,
    strictly finer than comparing ranks alone: two Jacobians can have the
    same rank while still being *inconsistent* under every possible scalar,
    which rank-comparison alone cannot detect but this can.

    Returns False if the two Jacobians are inconsistent under every scalar
    (safe to reject -- no unit omega can possibly work), True otherwise
    (must still attempt the full solve; this filter does not certify
    existence, only rules out impossible cases). Not applicable (returns
    True unconditionally) when K1=K2=1 (odd regime, no u/v at all) or when
    either constant term is nonzero (the fast path in
    solve_matching_at_component already handles that case exactly).
    """
    if A_c[0, 0] != 0 or B_c[0, 0] != 0:
        return True  # unsupported component: the exact fast path handles this case, not this filter
    pairs = []
    if K1 > 1:
        pairs.append((A_c[1, 0], X1[1, 0]))
        pairs.append((B_c[1, 0], X2[1, 0]))
    if K2 > 1:
        pairs.append((A_c[0, 1], X1[0, 1]))
        pairs.append((B_c[0, 1], X2[0, 1]))
    if not pairs:
        return True
    omega0 = None
    for src, tgt in pairs:
        if src != 0:
            ...
            if omega0 is None:
                ...
                return False  # two derivative directions demand different scalars -- impossible
        else:
            if tgt != 0:
                return False  # source vanishes here but target doesn't -- no scalar can fix this
    if omega0 is not None and omega0 == 0:
        return False  # the only consistent scalar is 0, but omega must be a unit
    return True
...
def solve_matching_at_component(A_c, B_c, X1, X2, D: int, K1: int, K2: int):
    ...
    Lemma 3.7's per-component matching equation).

    FAST PATH (no search at all): if A_c (or B_c) has a nonzero constant
    term, it is already a unit of the *whole* local ring J_c (not just its
    residue field) -- J_c is local with maximal ideal <u,v>, and any
    element outside <u,v> is a unit. Since J_c is commutative, omega*A_c=X1
    with A_c a unit has the UNIQUE solution omega = X1 * A_c^{-1}, computed
    by a full ring inversion (jc_inverse, Newton iteration -- NOT merely
    inverting the constant term, which only pins down omega's residue-field
    image and was an earlier, incorrect version of this optimization; see
    jc_inverse's docstring). We then VERIFY this omega against the *full*
    truncated equations (both A_c and B_c) -- so correctness does not
    depend on this reasoning being complete, only on the verification,
    which is exact and independent of it.
    This turns what was previously always a (2*D*K1*K2)-unknown linear
    solve into O(log(K1*K2)) ring multiplications for every component
    where it applies (typically the majority -- see README for measured
    counts and the ~2x measured speedup on the four benchmark codes).

    SLOW PATH (genuine search over an affine solution space): only
    reached when *both* A_c[0,0]=0 and B_c[0,0]=0 (both in the maximal
    ideal), where the constant-term equations are vacuous and a real
    linear system over J_c's full D*K1*K2 F2-coordinates must be solved
    and searched for a unit.
    ...
    def verify(omega):
        return (np.array_equal(jc_to_f2_vector(jc_mul(omega, A_c, K1, K2), D, K1, K2),
                                jc_to_f2_vector(X1, D, K1, K2))
                and np.array_equal(jc_to_f2_vector(jc_mul(omega, B_c, K1, K2), D, K1, K2),
                                    jc_to_f2_vector(X2, D, K1, K2))
                and jc_is_unit(omega))

    # --- fast path: A_c (or B_c) is a full unit of J_c, omega is forced ---
    if A_c[0, 0] != 0:
        A_c_inv = jc_inverse(A_c, K1, K2)
        omega = jc_mul(X1, A_c_inv, K1, K2)
        return omega if verify(omega) else None
    if B_c[0, 0] != 0:
        B_c_inv = jc_inverse(B_c, K1, K2)
        omega = jc_mul(X2, B_c_inv, K1, K2)
        return omega if verify(omega) else None

    # --- slow path: both constant terms are zero, genuine search needed ---
